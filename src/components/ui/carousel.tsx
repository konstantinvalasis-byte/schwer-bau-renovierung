...
const Slide = ({ slide, index, current, handleSlideClick }: SlideProps) => {
  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      ...
      >
        <div
          className="absolute top-0 left-0 w-full h-full bg-slate-900 rounded-[1%] overflow-hidden transition-all duration-150 ease-out"
          style={{}}
        >
          <img
...
const CarouselControl = ({ type, title, handleClick }: CarouselControlProps) => {
  return (
    <button
      className={`w-10 h-10 flex items-center mx-2 justify-center bg-white border border-slate-200 rounded-full focus:border-rose-600 focus:outline-none hover:-translate-y-0.5 active:translate-y-0.5 transition duration-200 hover:bg-rose-600 hover:text-white group ${
        type === "previous" ? "rotate-180" : ""
      }`}
      title={title}
      onClick={handleClick}
    >
      <IconArrowNarrowRight className="text-slate-600 group-hover:text-white transition-colors" />
    </button>
  );
};
